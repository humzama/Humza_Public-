...
extern int MACDx = 58;           //Buy 0 MACD range
...
extern int MAx = 55;             //Buy 1 MA range
extern int period1 = 25;         //Buy 1 MA Fast Period
extern int period2 = 133;        //Buy 1 MA Slow Period
extern int movrsiperiod = 23;    //Buy 1 RSI Period
// function 2 buying variables: Ichimoku
extern int ichirsiperiod = 20;   //Buy 2 RSI Period
extern int SenAv = 77;           //Buy 2 Senkou Average Period


// SELLING FUNCTIONS
extern int sellStopLoss = 200;   //Sell Stop Loss level
extern int rsiPeriod = 12;       //Sell RSI Period
extern int fractalPeriod = 10;   //Sell Check Fractal range
// function 3 variables - RsiFractalMacd
extern int rsiSupport3 = 20;     //Sell 3 RSI Support
extern int rsiResistance3 = 80;  //Sell 3 RSI Resistance
extern int macdFast = 10;        //Sell 3 MACD Fast EMA
extern int macdSlow = 25;        //Sell 3 MACD Slow EMA
extern int macdSignal = 7;       //Sell 3 MACD Signal EMA
// function 4 variables - RsiFractalKinko
extern int rsiSupport4 = 40;     //Sell 4 RSI Support
extern int rsiResistance4 = 65;  //Sell 4 RSI Resistance
extern int tenkanPeriod = 12;    //Sell 4 Tenkan Sen
extern int kijunPeriod = 27;     //Sell 4 Kijun Sen
extern int senkouBPeriod = 54;   //Sell 4 Senkou B
// function 5 variables - RsiFractalCandle
extern int rsiSupport5 = 30;     //Sell 5 RSI Support Level
extern int rsiResistance5 = 80;  //Sell 5 RSI Resistance Level
   ...
   if(Minute() == timec)      //if current minute == minute counter
   {
      ...
      {   
            ...
            {      
               ...
               CheckBuyRSIICHIMOKU();        //function 1
               CheckBuyRSIMA();              //function 2
...
{
   int total = OrdersHistoryTotal();      //# of closed orders in history
   
   if(total>orderCount)    //# if there are more closed orders than orderCount
   {
      for(int i=orderCount; i<total; i++)       //for every order that closed since the last check
      {
         if(OrderSelect(i,SELECT_BY_POS,MODE_HISTORY))
         {
            orderCount++;                          //increment orderCount
            string comment = OrderComment();       //get the order's comment
            double profit = OrderProfit();         //get the order's profit (negative if it's a loss)
            
            //FUNCTION 0 - BUY MACD
            if(StringCompare(StringSubstr(comment,0,4),"Macd",FALSE)==0)         //check function 0's comment against the order's comment
            {
               Print("*** CLOSED TRADE *** Order#: ",i+1,";  ",comment,";  Profit: ",profit);     //check to see profit or loss is correct
               if(profit < 0)                      //if it's a loss
               {
                  BuyLossRSIMACD += (profit/-1);   //negate the profit and add to the function's LOSS variable
               }//end if(profit < 0) 
               else                                //if it's a profit
               {
                  BuyProfitRSIMACD += profit;      //add the profit to the function's PROFIT variable
               }//end else
               
               if(BuyLossRSIMACD != 0)             //if function's LOSS variable is not zero (to prevent divide by 0 error)
               {
                  pf[0] = BuyProfitRSIMACD/BuyLossRSIMACD;     //update the function's profit factor
               }//end if(BuyLossRSIMACD != 0)
               
               Print("%%% PROFIT FACTOR %%% ",comment,";  Profit Factor: ", pf[0]);        //check to see if profit factor is being calculated
            }//end if(StringCompare(...))
            
            //FUNCTION 1 - BUY ICHIMOKU
            if(StringCompare(StringSubstr(comment,0,8),"Ichimoku",FALSE)==0)     //check function 0's comment against the order's comment
            {
               Print("*** CLOSED TRADE *** Order#: ",i+1,";  ",comment,";  Profit: ",profit);     //check to see profit or loss is correct
               if(profit < 0)
               {
         ...
         else        //if OrderSelect(...) is false
         {
            Alert(GetLastError());
         }//end else
      }//end for(...)
   }//end if(total>orderCount)
}//end CalculateProfitFactor()
//+------------------------------------------------------------------+